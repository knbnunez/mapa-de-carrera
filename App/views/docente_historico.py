...
class DocenteHistoricoView(TemplateView): # Detalle para un único docente
    ...
    def get(self, request, legajo): # Se puede recuperar el param atr llamándolo como esta definido en urls.py en este caso: legajo
        ...
        if (docente is None) and (Docente.objects.filter(legajo=legajo).exists()):
            ...
            DocenteHistoricoView.cargos_activos = None
        ...
        if (Cargo.objects.filter(docente=docente).exists()): # Caso en el que existe al menos un cargo en la BD, lo recupero
            cargos = Cargo.objects.filter(docente=docente).order_by('-nro_cargo')
            comisiones_cte_ch = []
            cargos_horas = []
            
            for c in cargos:
                cargos_historicos.append(c) # Los añadimos al diccionario que recibirá al template
        ...
        DocenteHistoricoView.cargos_activos = cargos_activos
        return render(request, self.template_name, {'docente': docente, 'cargos_historicos': cargos_historicos, 'comisiones_cte_ch': comisiones_cte_ch, 'cargos_horas': cargos_horas }) # materias, comisiones, tareas extras

        
    def post(self, request, legajo):
        for ca in DocenteHistoricoView.cargos_activos:
            # print(ca)
